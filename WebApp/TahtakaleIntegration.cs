...
namespace WebApp
{
    public class TahtakaleIntegration
    {
        ...
        public async Task<TahtaKaleResponseDto> GetDataWithPaging(int take, int skip)
        {
        ...
        public async Task<PagedResult<Item>> SearchForecastAsync(RequestArgs<Item> args, int amount = 10)
        {
            PagedResult<Item> pagedResult = null;
            Pager pager = null;

            if (args == null && pager == null)
                pager = new Pager(pageNr: 1, pageSize: 50, sortColumn: "", SortDirection.Descending); // generatedForecasts isn't cleared when leaving the server side example, so pager could be null
            else
                pager = args.Pager;

            // This is the important part
            Rss rss = await XmlToData();
            IQueryable<Item> result = rss.Channel.Items.AsQueryable();

            Expression<Func<Item, bool>>? filterExpression = null;
            List<Item> rsl = result.ToList();
            foreach (var item in rsl)
            {
                item.CategoryTypeStr = string.Join(',', item.Category);
                item.CategoryTypeStr = HttpUtility.UrlDecode(item.CategoryTypeStr);
                item.ProductTypeStr = string.Join(',', item.ProductType);
                item.ProductTypeStr = HttpUtility.UrlDecode(item.ProductTypeStr);
            }
            if (args != null)
            {
                foreach (var arg in args.AppliedFilters)
                {
                    if (arg.FilterType.UseEmbeddedFilter)
                        rsl = arg.FilterType.ApplyEmbeddedFilter<Item>(rsl, arg.PropertyName, arg.FilterValue.ToString());
                    else
                    {
