    ...
    public class TahtakaleIntegration
    {
        ...
        public async Task<PagedResult<Item>> SearchForecastAsync(RequestArgs<Item> args, int amount = 10)
        {
            ...
            if (args == null && pager == null)
                pager = new Pager(pageNr: 1, pageSize: 50, sortColumn: "", SortDirection.Ascending); // generatedForecasts isn't cleared when leaving the server side example, so pager could be null
            else
                pager = args.Pager;
